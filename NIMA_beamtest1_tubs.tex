...
\label{trigs}
Triggering using three 2cm x 2cm, 5mm thick plastic scintillator counters, readout by 2" PMTs via an air waveguide to remove the PMTs from direct beam exposure.




\subsection{Proton Beamline Description}
A proton test beam was conducted at NASA Space Radiation Laboratory (NSRL) facility at BNL. The following proton beam energies were used:
210MeV ($\sim$dE/dx of K+ from free proton decay)
475MeV (water Cerenkov thresh.)
2GeV ($\sim$minimum ionizing)
Low intensity narrow beam

Beam details:
Intensity ~1p+/bunch
Beam size ~1cm x1cm @ 2GeV; 5.4cm x 5.4cm @210MeV,
0.4s spill every ~4 sec.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
A baseline is defined as the average value of all the points in the first integration window (between the two red lines) that is 50ns wide. To check the baseline quality, its averaged value is compared agains the average of the third window right after the signal window (between two blue lines). Events with this difference larger then ~20 ADC bins are flagged as noise ones. This allows removal of the noise events or events with bad baseline. Additionally, an average of a window at the very beginning of the waveform (between 10ns and 40ns, not shown because the figure is zoomed around the signal area) is used for general baseline quality check using the above criterion.
